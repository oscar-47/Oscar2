              ...
              <div className="space-y-3">
                ...
                <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))' }}>
                  {cards.map((_, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-center rounded-2xl border border-[#e1e1e5] bg-white/70"
              ...
              <div className="space-y-3">
                <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))' }}>
                  {cards.map((c, i) => c.status === 'success' && c.url ? (
                    <div
                      key={i}
                      className="group relative overflow-hidden rounded-2xl border border-[#dcdce1] bg-white"
                      style={{ aspectRatio: previewAspectRatio }}
                    >
                    ...
                    <div key={i} className="rounded-2xl border border-destructive/40 bg-destructive/5 p-3 text-xs text-destructive">{c.error ?? tc('error')}</div>
                  ))}
                </div>
