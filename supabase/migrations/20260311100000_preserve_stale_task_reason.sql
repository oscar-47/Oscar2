...
begin
  ...
  set
    ...
    last_error = coalesce(
      nullif(t.last_error, ''),
      case
        when t.status = 'running' then 'TASK_STALE_NO_HEARTBEAT: worker stopped heartbeating while running'
        else 'MAX_ATTEMPTS_EXCEEDED: task retried too many times'
      end
    ),
    updated_at = now()
  ...
  set
    ...
    last_error = case
      when t.status = 'running'
        and coalesce(t.locked_at, now() - interval '10 minutes') <= now() - interval '3 minutes'
        then coalesce(nullif(t.last_error, ''), 'TASK_STALE_NO_HEARTBEAT: worker stopped heartbeating while running')
      else t.last_error
    end,
    updated_at = now()
  where t.job_id = p_job_id
